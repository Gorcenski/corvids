...
class TestRecreateData(unittest.TestCase):
    ...
    def test_integration_get_data_simple(self):
        ...
        self.assertEqual(expected_result, self.rd.getDataSimple(poss_vals)[expected_key])
    ...
    def test_integration_get_data_simple_with_multiple_results(self):
        ...
        solution, poss_vals = self.rd.recreateData()
        self.assertIn(expected_result_first, self.rd.getDataSimple(poss_vals)[expected_key])
        self.assertIn(expected_result_second, self.rd.getDataSimple(poss_vals)[expected_key])
    ...
    def test_integration_with_no_multiprocess(self):
        ...
        self.assertIn(expected_result_first, self.rd.getDataSimple(poss_vals)[expected_key])
        self.assertNotIn(expected_result_second, self.rd.getDataSimple(poss_vals)[expected_key])
    ...
    def test_integration_with_poss_vals(self):
        ...
        self.assertNotIn(expected_result_first, self.rd.getDataSimple(poss_vals)[expected_key])
        self.assertIn(expected_result_second, self.rd.getDataSimple(poss_vals)[expected_key])
    ...
    def test_integration_with_check_val_and_poss_vals(self):
        ...
        expected_result_second = [2, 2, 2, 4, 5]
        self.rd = RecreateData(min_score=1, max_score=5, num_samples=5, mean=3.0, variance=2.0)
        solution, poss_vals = self.rd.recreateData(check_val=5, poss_vals=[2, 3, 4, 5])
        self.assertNotIn(expected_result_first, self.rd.getDataSimple(poss_vals)[expected_key])
        self.assertIn(expected_result_second, self.rd.getDataSimple(poss_vals)[expected_key])
    ...
    def test__compute_valid_means_variances(self):
        ...
        self.assertEqual(expected_result, self.rd._compute_valid_means_variances(False, range(1, 6)))
    ...
    def test__compute_valid_means_variances_with_precision(self):
        ...
        expected_result = (3.0, 2.0)
        self.assertIn(expected_result, self.rd._compute_valid_means_variances(False, range(1, 6)))
    ...
    def test__compute_valid_means_variances_with_precision_and_find_first_true_with_none_result(self):
        ...
        self.assertIsNone(self.rd._compute_valid_means_variances(True, range(1, 51)))
    ...
    def test__build_solution_space_with_multiprocess(self):
        ...
        expected_result = [(Matrix([[1, 1, 0, 3, 0]]), 
                            ...
                            Matrix([[  5],
                                    ...
                                    [200]]),
                            (3.0, 2.0))]
        self.assertEquals(expected_result, self.rd._build_solution_space(mvp, range(1, 6), multiprocess=True))
    ...
    def test__build_solution_space_without_multiprocess(self):
        ...
        expected_result = [(Matrix([[1, 1, 0, 3, 0]]), 
                            ...
                            Matrix([[  5],
                                    ...
                                    [200]]),
                            (3.0, 2.0))]
        self.assertEquals(expected_result, self.rd._build_solution_space(mvp, range(1, 6), multiprocess=False))
    ...
    def test__build_solution_space_should_be_identical_with_or_without_multiprocess(self):
        ...
        self.assertEquals(rd1._build_solution_space(mvp1, range(1, 6), multiprocess=True),
    ...
    def test_multiprocess_get_solution_space(self):
        expected_result = (Matrix([[1, 1, 0, 3, 0]]), 
                           ...
                           Matrix([[  5],
                                   ...
                                   [200]]),
                           (3.0, 2.0))
        self.assertEquals(expected_result, multiprocess_get_solution_space(5, range(1, 6), (3.0, 2.0)))
    ...
    def test_multiprocess_get_solution_space_with_int_check_val(self):
        expected_result = (Matrix([[0, 3, 0, 1, 0]]), 
                           ...
                           Matrix([[  4],
                                   ...
                                   [100]]),
                           (3.0, 2.0))
        self.assertEquals(expected_result, multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val=5))

    def test_multiprocess_get_solution_space_with_single_list_check_val_same_as_scalar(self):
        self.assertEquals(multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val=5),
                          multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val=[5]))

    def test_multiprocess_get_solution_space_with_single_dict_check_val_same_as_scalar(self):
        self.assertEquals(multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val=5),
                          multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val={5: 1}))

    def test_multiprocess_get_solution_space_with_multiple_list_check_val(self):
        self.assertIsNone(multiprocess_get_solution_space(100, range(1,6), (2.3, 0.45), check_val=[1,5]))

    def test_multiprocess_get_solution_space_with_multiple_dict_check_val(self):
        self.assertIsNone(multiprocess_get_solution_space(100, range(1,6), (2.3, 0.45), check_val={5: 1, 1: 2}))
    ...
    def test_multiprocess_get_solution_space_with_poss_vals_and_list_check_val(self):
        self.assertIsNone(multiprocess_get_solution_space(5, range(1,6), (3.0, 2.0), check_val=[4,5]))

    # _find_first_solution
    def test__find_first_solution_independent_of_multiprocess(self):
        find_first = True
        mean_var_pairs = self.rd._compute_valid_means_variances(find_first, range(1, 6))
        solution_spaces = self.rd._build_solution_space(mean_var_pairs, range(1, 6))
        self.assertDictEqual(self.rd._find_first_solution(solution_spaces, range(1, 6), multiprocess=True)[0], 
                             self.rd._find_first_solution(solution_spaces, range(1, 6), multiprocess=False)[0])

    def test__find_first_solution_without_check_val(self):
        find_first = True
        poss_vals = [1, 2, 3, 4, 5]
        mean_var_pairs = self.rd._compute_valid_means_variances(find_first, poss_vals)
        solution_spaces = self.rd._build_solution_space(mean_var_pairs, poss_vals)
        expected_result = {'_': [[1, 1, 1, 1, 1]]}
        solution, poss_vals = self.rd._find_first_solution(solution_spaces, poss_vals)
    ...
    def test__find_first_solution_with_check_val_and_poss_vals(self):
        ...
        check_val = [4, 5]
        mean_var_pairs = self.rd._compute_valid_means_variances(find_first, poss_vals)
        solution_spaces = self.rd._build_solution_space(mean_var_pairs, poss_vals, check_val=check_val)
        expected_result = {'_': [[1, 1, 1, 1, 1]]}
        solution, poss_vals = self.rd._find_first_solution(solution_spaces, poss_vals, check_val=check_val)
