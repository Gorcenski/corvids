...
class TestRecreateData(unittest.TestCase):
    ...
    def test_valid_means_variances(self):
        self.rd = RecreateData(min_score=1, max_score=5, num_samples=5, mean=3.0, variance=2.0)
        self.rd.poss_vals = range(self.rd.absolute_min, self.rd.absolute_max+1)
        expected_result = [(3.0, 2.0)]
        self.assertEqual(expected_result, self.rd.validMeansVariances(False))

    def test_valid_means_variances_with_precision(self):
        ...
        self.rd.poss_vals = range(self.rd.absolute_min, self.rd.absolute_max+1)
        expected_result = (3.0, 2.0)
        self.assertIn(expected_result, self.rd.validMeansVariances(False))

    def test_valid_means_variances_with_precision_and_find_first_true_with_none_result(self):
        ...
        self.rd.poss_vals = range(self.rd.absolute_min, self.rd.absolute_max+1)
        self.assertIsNone(self.rd.validMeansVariances(True))
